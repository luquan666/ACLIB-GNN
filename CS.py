...
results_dir = 'results_CS'
os.makedirs(results_dir, exist_ok=True)

dataset = Coauthor(root='/tmp/CoauthorCS', name='CS')
...
for fold, (train_idx, test_idx) in enumerate(skf.split(data.x.cpu(), data.y.cpu())):
    ...
    for epoch in pbar:
        ...
        with torch.no_grad():
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                torch.save(model.state_dict(), f'{results_dir}/best_fold{fold}_alpha{alpha}.pth')  # 这里参数修改

            pbar.set_postfix({'loss': total_loss.item(), 'val_acc': val_acc})

    model.load_state_dict(torch.load(f'{results_dir}/best_fold{fold}_alpha{alpha}.pth'))  # 这里参数修改
    model.eval()
    with torch.no_grad():
        node_pred = model(data.x, data.edge_index)
